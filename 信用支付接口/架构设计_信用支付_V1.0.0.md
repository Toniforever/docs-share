...
该接口用于游戏在启动后调用，用于判断该用户是否有欠款到期需要偿还。如果有到期欠款的，返回欠款金额，如果无到期欠款，则返回0。
>note:
>1)如果该用户未登录，接口返回0，表示这个无法识别的用户没有欠款。
>
...
http请求方式：POST
    ...
    http://183.232.65.120:8882/pycc/ExpireArrears
POST数据格式：JSON
...
正常时的返回JSON数据包示例：
    ...
    {"errcode":0,"errmsg":"获取用户是否有到期欠款成功","isBalanceDue":"true","sum":1000}
...
sum|是|欠费金额，单位：分。当isBalanceDue为“true”时，该参数值有效。
...
http请求方式：POST
    ...
    http://IP:PORT/if/credit_billing/isBalanceDue
